...
public class Main {
    ...
    private static int maxPopulationReachable(List<List<Integer>> adjacencyList, int[][] capacities, int source,
            int sink, int numberOfLinks) {
        int maxFlow = 0;
        int[] parent = new int[adjacencyList.size()];
        int cont = numberOfLinks;
        int n = 0;
        // to check given number of link odd or even
        if (cont > 1) {
            if (cont % 2 == 0) {
                n = 0;
            } else {
        ...
        while (bfs(adjacencyList, capacities, parent, source, sink) && cont >= n) {
    ...
    private static boolean bfs(List<List<Integer>> adjacencyList, int[][] capacities, int[] parent, int source,
            int sink) {
