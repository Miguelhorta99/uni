...
function getAnuncios() {
    ...
    $.post(actionUrl, {tipo: "oferta"}, function (tipo) {
        ...
        for (let a = 0; a < 3; a++) {
            ...
            appearanceOfAnuncios(tipo.list[a].anunciante,
                    tipo.list[a].id, tipo.list[a].zona, tipo.list[a].date, tipo.list[a].estado, "anuncios");
...
function getDetalhesAnuncios(aidAnuncio) {
    ...
    $.post(actionUrl, {aid: id}, function (aid) {
...
$(document).ready(function () {
    ...
    $("#login_regist_button").click(function(){
        ...
        $("#enviarMensagemAoAnunciante").hide();
        $("#anuncios").hide();
        $("#utlizador_registado_button").hide();
